...
class DrivableMap {
    ...
    public boolean addDrivable(String id, Drivable d){
        if (drivable_map.containsKey(id) == false){
    ...
    public boolean hasFasterThan(int speed) {
        List<Drivable> l = new ArrayList<Drivable>(drivable_map.values());
        for (int i = 0; i < l.size(); i++){
            if (l.get(i).getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public ArrayList<Tradable> getTradable() {
        ...
        List<Drivable> l = new ArrayList(drivable_map.values());
        for (int i = 0; i < l.size(); i++){
            if (l.get(i) instanceof Tradable){
                Tradable t = (Tradable) l.get(i);
                toReturn.add(t);
            }
        }
        return toReturn;
